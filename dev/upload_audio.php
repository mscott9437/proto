<?php
	...
	if(!is_uploaded_file($_FILES['upload']['tmp_name'])){
		...
		exit();}
	if($_FILES['upload']['size']>32000000){
		header('Location:error.php?message=limit');
		exit();}
	if(mime_content_type($_FILES['upload']['tmp_name'])!="audio/x-wav"&&mime_content_type($_FILES['upload']['tmp_name'])!="audio/mpeg"&&mime_content_type($_FILES['upload']['tmp_name'])!="application/ogg"){
		header('Location:error.php?message=format');
		exit();}

	$uploadDir=DOC_ROOT."/../profile/".$_SESSION['type']."_audio/".$_SESSION['name']."/";
	$now=time();
	...
	if(mysqli_stmt_prepare($stmt,"INSERT INTO ".$_SESSION['type']."_audio(head,path,link) VALUES(?,?,?)")){
